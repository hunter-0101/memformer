...
class BasicConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, groups=1, gamma=0.9):
        ...
        self.access_freq = torch.zeros(32).long() 
    ...
    def fw_chunks(self):
        ...
        if not hasattr(self, 'access_freq'):
            setattr(self, 'access_freq', torch.zeros(self.W.size(1)).long())
        ...
        # forget
        threshold = 5  # to be optimized
        low_freq_idx = (self.access_freq < threshold).nonzero(as_tuple=False).squeeze()
        self.W.data[:, low_freq_idx] = torch.randn_like(self.W.data[:, low_freq_idx])
        self.access_freq[low_freq_idx] = 0
        ...
        if self.trigger == 1:
            ...
            # Update access frequency for memory slots
            self.access_freq[idx.squeeze().long()] += 1
            ...
            # Dynamically modify self.tau based on access frequency (priority)
            access_priority = torch.clamp(self.access_freq.float() / self.access_freq.max().float(), 0, 1)
            dynamic_tau = self.tau * (1 - access_priority[idx.squeeze().long()])

            w = dynamic_tau * w + (1-dynamic_tau) * o_w.view(w.size())
            b = dynamic_tau * b + (1-dynamic_tau) * o_b.view(b.size())
            f = dynamic_tau * f + (1-dynamic_tau) * o_f.view(f.size())
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dilation, final=False, gamma=0.9):
        super().__init__()
        self.conv1 = BasicConv(in_channels, out_channels, kernel_size, dilation=dilation, gamma=gamma)
        self.conv2 = BasicConv(out_channels, out_channels, kernel_size, dilation=dilation, gamma=gamma)
...
class MemTCN(nn.Module):
    def __init__(self, input_dims, output_dims, hidden_dims=64, depth=10, gamma=0.9):
        super().__init__()
