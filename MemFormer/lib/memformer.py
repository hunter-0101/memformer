...
from MemFormer.lib.data import batch_generator
from MemFormer.utils import extract_time, random_generator, NormMinMax
from MemFormer.lib.model import Encoder, Recovery, Generator, Discriminator, Supervisor
from MemFormer.lib.discriminative_metrics import discriminative_score_metrics
from MemFormer.lib.predictive_metrics import predictive_score_metrics
...
class BaseModel():
    ...
    def frozen_generation(self, noise):
        self.Z = torch.tensor(noise, dtype=torch.float32)
        self.E_hat = self.netg(self.Z)   
        self.H_hat = self.nets(self.E_hat)
        h_temp = self.H_hat
        generated_data_curr = self.netr(
        ...
        generated_data = generated_data * self.max_val
        generated_data = generated_data + self.min_val
        return h_temp, generated_data


class MemFormer(BaseModel):
    @property
    def name(self):
        return 'TimeGAN'

    def __init__(self, opt, ori_data):
        super(MemFormer, self).__init__(opt, ori_data)
    ...
    def optimize_params_er(self):
        ...
        self.backward_er()
        #######################################################################
        self.nete.store_grad()
        self.netr.store_grad()
        #######################################################################
